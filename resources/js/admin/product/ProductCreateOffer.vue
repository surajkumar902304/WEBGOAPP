<template>
  <div>
    ...
    <v-container fluid>
        <v-row class="mt-0 pt-0">
          ...
          <v-col cols="12" md="1" class="text-end p-0 ps-2">
            <v-btn color="secondary" small class="text-none w-100" style="height: 32px; color: #1976d2; font-weight: bold; background-color: white !important; 
              border: 1px solid #1976d2 !important;" @click="openAddDialog">
              Add Offers
            </v-btn>
          </v-col>
        </v-row>
    ...
    <v-row class="mt-0">
        <v-col cols="12">
            <v-card outlined>
              <v-data-table v-model="selectedtag" item-key="product_offer_id" :show-select="true" :items="productOffers" :headers="offerheaders" :search="ssearch" 
                ...
                <template #item.actions1="{ item }">
                    <div class="text-center">
                        <v-chip color="primary" class="white--text" outlined pill small @click="openEditDialog(item)" style="cursor: pointer;">
    ...
    <v-dialog v-model="addDialog" max-width="620" @update:model-value="resetAdd">
      <v-card>
        ...
        <v-form v-model="addValid" @submit.prevent="saveAdd">
          <v-card-text>
            <v-autocomplete v-model="addForm.product_ids" :items="products" item-text="mproduct_title" item-value="mproduct_id" label="Products" multiple small-chips 
              deletable-chips :rules="[required]" @change="loadAddVariants"/>
            <v-autocomplete v-model="addForm.variant_ids" :items="addVariants" item-text="option_label" item-value="mvariant_id" label="Variants" multiple small-chips 
              deletable-chips :rules="[required]"/>
            <v-text-field v-model="addForm.product_deal_tag" label="Product Deal Tag"/>
            <v-text-field v-model="addForm.product_offer" label="Product Offer"/>
          </v-card-text>
          <v-card-actions>
            <v-btn type="submit" color="success" :disabled="!addValid || saving">Add Offer</v-btn>
          </v-card-actions>
        </v-form>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="editDialog" max-width="620" @update:model-value="resetEdit">
      <v-card>
        ...
        <v-form v-model="editValid" @submit.prevent="saveEdit">
          <v-card-text>
            <v-autocomplete v-model="editForm.product_id" :items="products" item-text="mproduct_title" item-value="mproduct_id" label="Product" :rules="[required]"
              @change="loadEditVariants"/>
            <v-autocomplete v-model="editForm.variant_id" :items="editVariants" item-text="option_label" item-value="mvariant_id" label="Variant" :rules="[required]" 
              clearable/>
    ...
    <v-dialog v-model="bulkDeleteDialog" max-width="400">
        <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn text color="red" :loading="bulkDeleteLoading" :disabled="bulkDeleteLoading" @click="performBulkDelete">Delete</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="bulkAddDialog" max-width="500">
      <v-card>
        ...
        <v-card-text>
          <v-text-field v-model="bulk_product_tag" label="Product tag" :rules="[ v => !!v || 'Tag is required',   v => (v && v.length >= 3) || 'Minimum 3 characters required' ]" 
            required/>
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn text @click="bulkAddDialog = false">Cancel</v-btn>
          <v-btn text color="red" :loading="bulkAddLoading" :disabled="bulkAddLoading || !bulk_product_tag" @click="performBulkAdd">Add</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="bulkRemoveDialog" max-width="500">
      <v-card>
        ...
        <v-card-actions>
            ...
            <v-btn text color="red" :loading="bulkRemoveLoading" :disabled="bulkRemoveLoading" @click="performBulkRemove">Remove</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
