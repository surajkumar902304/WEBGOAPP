...
<div>
            ...
            <v-card outlined>
                <v-data-table v-model="selected" :show-select="true" item-key="msubcat_id" :items="subcats" :headers="msubcatsHeaders" :search="ssearch" :footer-props="{
                    ...
                    <template v-slot:item.msubcat_name="{ item }">
                        <router-link :to="{ name: 'edit-subcat', params: { msubcatid: item.msubcat_id } }" class="link-dark">
                            {{ item.msubcat_name }}
                        </router-link>  
                    </template>
    ...
    <v-dialog v-model="bulkDeleteDialog" max-width="400">
        <v-card>
            <v-card-title class="text-h6">Confirm Delete</v-card-title>
            <v-card-text>Are you sure you want to delete <strong>{{ selected.length }}</strong> sub-categories?</v-card-text>
            <v-card-actions>
                <v-spacer></v-spacer>
