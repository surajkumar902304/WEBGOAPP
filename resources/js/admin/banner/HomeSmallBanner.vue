<template>
    <div>
      <v-row>
        <h2 class="text-h6 mb-1">Small Sliders</h2>
      </v-row>
      <v-container fluid>
          <v-row class="mt-0 pt-0 mb-3 small-search">
        ...
        <v-col cols="12" class="pt-0">
          <v-card outlined>
            <v-simple-table>
              ...
              <draggable tag="tbody" :list="smallbanners" handle=".drag-handle" @end="onDragEnd">
                <tr v-for="item in filteredBanners" :key="item.home_small_banner_id">
                  <td class="p-2">
                    <img :src="cdn + item.home_small_banner_image || 'https://via.placeholder.com/50'" width="100" height="75" />
                  </td>
                  <td class="align-middle">
                    {{ item.home_small_banner_name }}
                  </td>
        ...
        <v-card>
          ...
          <v-form v-model="fsvalid" @submit.prevent="saveBanner">
            <v-card-text>
              ...
              <v-text-field v-model="defaultItem.home_small_banner_name" :rules="bannernameRule" label="Slider Name"/>
...
export default {
  ...
  data () {
    return {
      ...
      fillLock       : false,       
      defaultItem    : {
        home_small_banner_id   : null,
        home_small_banner_name : '',
        home_small_banner_image: '',
        main_mcat_id      : null,
        mcat_id           : null,
        msubcat_id        : null,
        mproduct_id       : null
  ...
  computed : {
    ...
    filteredBanners () {
      ...
      return this.smallbanners.filter(b =>
        (b.home_small_banner_name || '').toLowerCase().includes(term)
      )
    }
  },
  ...
  methods : {
    ...
    async loadBanners () {
      const { data } = await axios.get('/admin/small-banners/vlist')
      this.smallbanners = data.home_small_banner
    },
    ensureCats () {
    ...
    async editItem (item) {
      ...
      this.defaultItem = {
        home_small_banner_id   : item.home_small_banner_id,
        home_small_banner_name : item.home_small_banner_name,
        home_small_banner_image: '',
      ...
      this.imagePreview = this.cdn + item.home_small_banner_image
      this.imageName    = item.home_small_banner_image.split('/').pop()
      this.editedIndex  = item.home_small_banner_id
      this.fsvalid      = true
      this.addSdialog   = true
    },
    triggerFileInput () { 
    ...
    handleImageUpload (e) {
      ...
      if (!file) return
      this.defaultItem.home_small_banner_image = file
    ...
    async saveBanner () {
      ...
      fd.append('home_small_banner_name', this.defaultItem.home_small_banner_name)
      if (this.defaultItem.home_small_banner_image instanceof File)
        fd.append('home_small_banner_image', this.defaultItem.home_small_banner_image)
      if (this.editedIndex !== -1)
        fd.append('home_small_banner_id', this.editedIndex)

      const isNew = this.editedIndex === -1
      const url   = isNew ? '/admin/small-banners/add'
                          : '/admin/small-banners/update'
    ...
    async onDragEnd () {
      const payload = this.smallbanners.map((it,i)=>({id:it.home_small_banner_id,position:i+1}))
      try {
        await axios.post('/admin/small-banners/reorder', payload)
    ...
    async performDelete () {
      ...
      try {
        await axios.post('/admin/small-banners-delete',
                         {home_small_banner_id:this.browseBannerToDelete.home_small_banner_id})
